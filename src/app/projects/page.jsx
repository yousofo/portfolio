...
export default function Projects() {
  ...
  return (
    <div className="flex flex-col justify-center items-center h-full max-w-3xl mx-auto">
      ...
      <motion.div initial={false} animate={true ? "open" : "closed"} custom={dimentions.height} variants={navVariants} ref={containerRef} className="grid grid-cols-2 gap-2 w-full h-full">
...
const ProjectCard = ({ data }) => {
  return (
    <motion.div variants={itemVariants} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} className="w-full h-full bg-amber-400/10">
      <div>{data.title}</div>
    </motion.div>
  );
};
